...
export async function resolvePublicPhotoKey(clubSlug: string, albumSlug: string, photoId: number) {
...
export async function getPublicAlbumWithPhotos(clubSlug: string, albumSlug: string) {
  const result = await db
    .select({
      albumId: albums.id,
